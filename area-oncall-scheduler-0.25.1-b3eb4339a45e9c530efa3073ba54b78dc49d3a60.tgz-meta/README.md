...
--start 2021-08-01 \
--end 2021-08-01 \
--scheduler-pagerduty-client-id xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx \
--scheduler-pagerduty-client-secret xxxxx+xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx \
--scheduler-pagerduty-schedule-id PXXXXXX \
-v 3 \
--logtostderr=true \
...
To apply the shifts to PagerDuty, add the `--write` flag
...
- `Schedulers`: this is an abstraction over an external shift management tool. The goal of this interface is to 
  ...
  The only implementation so far uses `PagerDuty` as a backend.
...
1. Get a list of members of the rotation from PagerDuty
...
4. Write shift override to PagerDuty
